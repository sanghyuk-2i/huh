...
::: tip
  커스텀 타입(`BANNER`, `SNACKBAR` 등)도 자유롭게 사용할 수 있습니다. 소문자로 입력해도 자동으로
  대문자로 변환됩니다.
:::
...
::: tip
  커스텀 액션 타입(`OPEN_CHAT`, `SHARE` 등)도 사용할 수 있습니다. `HuhProvider`의 `onCustomAction`
  콜백에서 처리합니다.
:::

## 템플릿 변수

메시지, 제목, 액션 라벨, 액션 타겟에 `{{변수명}}` 형태의 템플릿 변수를 사용할 수 있습니다. 변수는 런타임에 `huh` 호출 시 전달됩니다.
