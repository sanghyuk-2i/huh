...
<Step title="2. Integration 생성 및 토큰 발급">
...
<Step title="3. 데이터베이스에 Integration 연결">
...
<Warning>Integration을 연결하지 않으면 API 호출 시 `404` 에러가 발생합니다.</Warning>

</Step>

<Step title="4. Database ID 확인">
...
<Note>Database ID는 하이픈을 포함해도, 포함하지 않아도 됩니다. API에서 자동으로 처리합니다.</Note>
