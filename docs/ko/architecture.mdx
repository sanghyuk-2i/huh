...
@huh/cli  ─── pull 명령어
    ...
    ├─ (i18n) @huh/core의 validateLocales로 cross-locale 검증
    │
    ▼
...
| 패키지                | 설명                                                                |
| --------------------- | ------------------------------------------------------------------- |
| `@huh/plugin-sentry`  | Sentry 연동 — 에러 시 `captureMessage`, 액션 시 `addBreadcrumb`     |
| `@huh/plugin-datadog` | Datadog 연동 — 에러 시 `datadogLogs.logger`, 액션 시 info 레벨 로깅 |
