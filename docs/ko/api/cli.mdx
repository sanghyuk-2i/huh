...
<Step title="데이터 fetch">
  Adapter 패턴으로 설정된 데이터 소스에서 데이터를 가져옵니다.
  i18n 모드에서는 각 로케일별로 데이터를 가져옵니다.
</Step>

<Step title="JSON DSL 변환">
  `parseSheetData`로 가져온 데이터를 JSON DSL 형식으로 변환합니다.
</Step>

<Step title="유효성 검증">
  `validateConfig`로 각 로케일별 검증을 수행합니다.
  i18n 모드에서는 추가로 `validateLocales`로 cross-locale 검증을 수행합니다.
  - 경고만 있으면 출력 후 계속 진행
  - 에러가 있으면 에러 출력 후 종료 (exit code 1)
</Step>

<Step title="JSON 파일 생성">
  지정된 output 경로에 JSON 파일을 생성합니다.
  i18n 모드에서는 로케일별 JSON 파일과 `index.ts` barrel 파일을 생성합니다.
</Step>
...
<Warning>
API 토큰(`GOOGLE_API_KEY`, `AIRTABLE_TOKEN`, `NOTION_TOKEN`)은 반드시 환경변수나 CI/CD 시크릿으로 관리하세요. 설정 파일에 토큰을 직접 넣는 경우 `.gitignore`에 해당 파일을 추가하여 저장소에 커밋되지 않도록 해야 합니다.
</Warning>
