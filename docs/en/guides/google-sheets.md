...
::: tip
  Custom types (`BANNER`, `SNACKBAR`, etc.) can also be used freely. Even if entered in lowercase,
...
::: tip
  Custom action types (`OPEN_CHAT`, `SHARE`, etc.) can also be used. Handle them via the
  `HuhProvider`'s `onCustomAction` callback.
:::

## Template Variables

You can use template variables in the format `{{variableName}}` in messages, titles, action labels, and action targets. Variables are passed at runtime when calling `huh`.
...
| ERR_FORBIDDEN | PAGE  | \{\{userName\}\} does not have access permission | Access Denied  | /img/403.png | CRITICAL | Go Back     | BACK       |              |
