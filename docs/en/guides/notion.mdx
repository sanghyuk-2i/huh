...
<Note>After duplicating, edit the data and run `huh pull`.</Note>
...
<Warning>If the integration is not connected, API calls will return a `404` error.</Warning>
...
<Note>The Database ID can include hyphens or not. The API handles both formats automatically.</Note>
