...
<Step title="Load ErrorConfig">
  Reads the JSON file from the `--config` path and parses it as ErrorConfig.
  If a `simulate` section exists in `.huh.config.json`, it is also loaded.
</Step>
...
<Step title="Start preview server">
  Starts an internal HTTP server that renders each error entry as an isolated HTML page.
  Built-in renderers for TOAST, MODAL, and PAGE types are applied automatically.
</Step>

<Step title="Capture screenshots with Playwright">
  Headless Chromium visits each error entry page and captures a screenshot.
  Type-specific delays are applied:
  - **TOAST**: 350ms
  - **MODAL**: 250ms
  - **PAGE**: 400ms
</Step>
...
<Step title="Extended Validation">
  Runs `@huh/core` base validation plus additional extended checks.
  See the [Extended Validation section](#extended-validation) below.
</Step>
...
<Note>
When a trackId has entries in `variables`, those values override the same keys in `defaultVariables`.
For example, with the config above, `ERR_AUTH`'s `userName` will be "Admin", while all other trackIds will use "Jane".
</Note>
...
`huh test` runs `@huh/core`'s base validation plus the following extended checks:
