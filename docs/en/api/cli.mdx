...
<Step title="Fetch data">
  ...
  In i18n mode, data is fetched for each locale separately.
</Step>
...
<Step title="Validate">
  Validate each locale with `validateConfig`.
  In i18n mode, additionally runs `validateLocales` for cross-locale validation.
...
<Step title="Generate JSON file">
  ...
  In i18n mode, generates per-locale JSON files and an `index.ts` barrel file.
</Step>
