...
|------|------|-------------|
| `rows` | `string[][]` | 2D string array where the first row is the header |
...
| `trackId` | `string` | Error ID to look up |
| `variables` | `Record<string, string>` | Template variables (optional) |
...
| `variables` | `Record<string, string>` | Variable map to substitute |
